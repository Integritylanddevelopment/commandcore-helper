from chatgpt_organizer.utils.agent_utils import run_agent, batch_and_throttle_requests

AGENT_CONFIG = {
    "agent_id": "team_meeting_bot",
    "style": "structured",
    "system_prompt": (
        "You are a team meeting bot. Your task is to facilitate team meetings by summarizing discussions, tracking action items, and ensuring "
        "effective communication among participants."
    ),
    "output_format": "markdown",
}

def preprocess_prompt(prompt: str) -> str:
    """Format the input prompt for team meeting facilitation."""
    return f"Meeting context: {prompt.strip()}"

def postprocess_result(result: str) -> str:
    """Format the result into actionable meeting summaries and action items."""
    return f"### Meeting Summary\n{result.strip()}"

def run(prompt: str):
    # Preprocess the input prompt
    processed_prompt = preprocess_prompt(prompt)

    # Execute the agent logic with batching and throttling
    result = batch_and_throttle_requests(lambda: run_agent(processed_prompt, AGENT_CONFIG))

    # Postprocess the result
    return postprocess_result(result)
