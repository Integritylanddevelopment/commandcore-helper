...
def sync_assistant_data(thread_id, project_name="gpt_imports", chat_name="chat_from_gpt", retries=3):
    logging.info(f"Syncing assistant data for thread '{thread_id}' in project '{project_name}'...")
    ...
    try:
        response = batch_and_throttle_requests(
            lambda: openai.ChatCompletion.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are an assistant sync bot."},
                    {"role": "user", "content": f"Sync data for thread '{thread_id}' in project '{project_name}' and chat '{chat_name}'."}
                ]
            )
        )
